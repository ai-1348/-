...
│ 📞 Reject Message: ${userConfig.REJECT_MSG ? 'Custom' : 'Default'}
│
│ ══════════════════
│
...
│ 👋 Welcome Message: ${userConfig.WELCOME_MESSAGE ? 'Set ✓' : 'Default'}
│ 👋 Goodbye Message: ${userConfig.GOODBYE_MESSAGE ? 'Set ✓' : 'Default'}
│
│ ══════════════════
│
│ 👥 *Permission Lists*
...
cmd({
    pattern: "antiedit",
    alias: ["editedit", "editblock"],
    desc: "Toggle anti-edit message protection",
    category: "settings",
    react: "📝",
    filename: __filename
},
async (conn, mek, m, { from, reply, isCreator, args, prefix, updateUserConfig, userConfig, sanitizedNumber }) => {
    ...
    if (!args[0]) {
        return reply(`📌 *Usᴀɢᴇ:* antiedit on/off\n*Cᴜʀʀᴇɴᴛ:* ${userConfig.ANTI_EDIT || 'false'}`);
    }
    ...
    await reply(`✅ *Aɴᴛɪ-Eᴅɪᴛ sᴇᴛ ᴛᴏ:* ${newValue}\n\nWhen ON: Bot will detect and notify when messages are edited.`);
});
